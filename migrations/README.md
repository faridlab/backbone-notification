# Database Migrations for notification

This directory contains SQLx database migrations for the notification module. These migrations create a comprehensive database schema with support for:
...
### 001_create_notification_table.sql
Creates the main `notification` table with:
...
### 002_create_notification_events_table.sql
Creates the event store table `notification_events` with:
...
### 003_create_notification_audit_table.sql
Creates the audit trail table `notification_audit` with:
...
### 004_create_notification_indexes_and_triggers.sql
...
### 005_create_notification_views.sql
Creates optimized database views:
- `notification_active` - Only active entities
- `notification_stats` - Materialized statistics view
- `notification_with_activity` - Entities with recent activity
- `notification_audit_summary` - Audit summary per entity
- `notification_event_timeline` - Chronological event view
- `notification_audit_timeline` - Chronological audit view
- `notification_by_metadata` - Entities with extracted metadata
- `notification_performance_metrics` - Daily performance metrics
...
```
notification
├── Main Table: notification
├── Event Store: notification_events
├── Audit Trail: notification_audit
├── Views (Optimized):
│   ├── notification_active
│   ├── notification_stats (materialized)
│   ├── notification_with_activity
│   ├── notification_audit_summary
│   ├── notification_event_timeline
│   ├── notification_audit_timeline
│   ├── notification_by_metadata
│   └── notification_performance_metrics (materialized)
└── Functions:
    ├── search_notification()
    ├── get_notification_statistics()
    ├── refresh_notification_materialized_views()
    └── schedule_notification_refresh()
```
...
FROM pg_matviews
WHERE matviewname LIKE 'notification_%';
