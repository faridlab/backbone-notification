...
/// Domain policy for Notification — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Notification>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type NotificationDomainPolicy = PermitAllPolicy<Notification>;
