...
/// Create stateless routes for the Notification module.
///
...
///
/// ```ignore
/// let module = NotificationModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::NotificationModule) -> Router<()> {
    Router::new()
...
/// Read-only routes for the Notification module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_notification_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_notification_routes(module: &crate::NotificationModule) -> Router<()> {
    Router::new()
        .merge(create_notification_read_routes(module.notification_service.clone()))
        .merge(create_notification_template_read_routes(module.notification_template_service.clone()))
}

/// Get all routes (stateless) for the Notification module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::NotificationModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Notification module.
///
...
pub fn create_combined_routes(module: &crate::NotificationModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Notification module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::NotificationModule) -> Router<AppState> {
    Router::new()
