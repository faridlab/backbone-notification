...
```bash
# Generate everything
backbone schema generate notification --target all

# Or generate specific targets
backbone schema generate notification --target proto,rust,sql
backbone schema generate notification --target handler,grpc,cqrs
backbone schema generate notification --target repository,events
```
...
```rust
use backbone_notification::NotificationModule;

// Create module instance with builder pattern
let module = NotificationModule::builder()
    .with_database(pool)
    .build()?;
...
2. Run generator: `backbone schema generate notification --target all`
...
2. Generate migration: `backbone migration alter {Entity} notification -d "description"`
3. Regenerate code: `backbone schema generate notification --target all`
4. Run migrations
...
```bash
# Run all tests
cargo test --package backbone-notification

# Run with database
DATABASE_URL=postgresql://... cargo test --package backbone-notification
```
